...
def _fast_struct_validate_record(rec: dict, schema: dict) -> None:
    ...
    for name, spec in properties.items():
        ...
        if spec.get("type") == "array" and spec.get("items", {}).get("type") == "string":
            items = spec["items"]["type"]
            for item in value:
...
def _struct_validate(records: list[dict], schema: dict, artifact: str) -> None:
...
def _validate_records(records: list[dict], schema: dict, artifact: str) -> None:
    ...
    if sample_size:
        ...
        sample = [records[i] for i in range(0, len(records), step)][:sample_size]
...
def build_normalized_reports(pairs: pd.DataFrame, cases: pd.DataFrame, config: PipelineConfig) -> list[dict]:
    """Render the shared 'Normalized Report' records (one row per PS drug per report)."""

    # reactions per report (case-level, all PTs observed in that report)
    reactions = (
    ...
    # Choose the drug rows deterministically: first by sort of primaryid, drug_name
    drug_rows = (
        pairs[["primaryid", "drug_name"]]
        .drop_duplicates()
        .sort_values(["primaryid", "drug_name"], kind="mergesort")
    ...
    for pid, drug in zip(pids, drugs):
        age = (case_lookup or {}).get("patient_age", {}).get(pid) if case_lookup else None
        sex = (case_lookup or {}).get("patient_sex", {}).get(pid) if case_lookup else None
        event_date = (case_lookup or {}).get("event_date", {}).get(pid) if case_lookup else None
        age_val = None
        if age is not None and not (isinstance(age, float) and _math.isnan(age)):
            age_val = float(age)
        elif age is not None:
            age_val = None
        sex_val = None if sex is None or pd.isna(sex) else str(sex)
        date_val = None if event_date is None or pd.isna(event_date) else str(event_date)
        append(
            {
                "report_id": pid,
                "drug_name": str(drug),
    ...
    with open(config.schema_path("normalized_report"), "r", encoding="utf-8") as fh:
        schema = json.load(fh)
    _validate_records(records, schema, "normalized_reports")
    return records


def build_candidate_signals(candidates: pd.DataFrame, config: PipelineConfig, schema: dict) -> list[dict]:
    """Translate candidate signal rows into schema-conformant JSON records."""

    records = []
    for _, r in candidates.sort_values("signal_id", kind="mergesort").iterrows():
        ...
        records.append(
            {
                ...
                "supporting_report_count": int(r["supporting_report_count"]),
                "prr": _num(r["prr"]),
                "ror": _num(r["ror"]),
                "trend_score": None,
                "risk_score": None,
    ...
    _validate_records(records, schema, "candidate_signals")
    return records
...
def write_datasets(config: PipelineConfig, **frames: pd.DataFrame) -> Path:
    """Write internal CSV artifacts with deterministic column order/sort."""

    config.output_dir.mkdir(parents=True, exist_ok=True)
    for name, df in frames.items():
        frame = _sortable_frame(df)
        sorted_df = frame.sort_values(list(frame.columns), kind="mergesort").reset_index(drop=True)
        path = config.output_dir / f"{name}.csv"
        sorted_df.to_csv(path, index=False, encoding="utf-8")
    return config.output_dir


def write_json(config: PipelineConfig, name: str, payload: dict | list) -> Path:
    config.output_dir.mkdir(parents=True, exist_ok=True)
...
def write_quality_report(config: PipelineConfig, ledger: QualityLedger) -> Path:
    return write_json(config, "data_quality_report", ledger.to_dict())
...
def write_pipeline_manifest(
    ...
    payload.setdefault("pipeline_version", "0.1.0")
    return write_json(config, "pipeline_manifest", payload)